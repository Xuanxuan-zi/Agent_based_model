...
class SugarAgent(CellAgent):
    ...
    def __init__(self, model, cell, sugar=0, metabolism=0, vision = 0):
        ...
        # ===== MODIFICATION: Add boredom mechanism to agent decision-making =====
        # Track how many steps agent has stayed in current location
        self.steps_in_current_location = 0
        # Random boredom threshold: each agent has different patience (5-15 steps)
        self.boredom_threshold = self.random.randint(5, 15)
        
    ## Define movement action with boredom mechanic
    def move(self):
        # ===== MODIFICATION: Check boredom before deciding movement strategy =====
        # If agent is bored (stayed too long in one place), it MUST move
        # Otherwise, use original greedy strategy
        
        if self.steps_in_current_location > self.boredom_threshold:
            # Agent is bored! Force movement to any available cell (not greedy)
            possibles = [
                cell
            ...
            if possibles:
                ...
                # Reset boredom counter after moving
                self.steps_in_current_location = 0
        else:
            # Agent is not bored yet, use original greedy logic
            ...
            # ===== MODIFICATION: Update boredom counter after movement =====
            # If agent moved to a different location, reset boredom counter
            # If agent stayed in same location (couldn't find better), increment counter
            if self.cell == old_cell:
                self.steps_in_current_location += 1
            else:
                self.steps_in_current_location = 0
                
    ## Consumer sugar in current cell, depleting it, then consume metabolism
